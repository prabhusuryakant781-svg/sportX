...
export function checkKneeValgus(hipL, hipR, kneeL, kneeR, ankleL, ankleR) {
  ...
  if (kneeWidth < ankleWidth * 0.78 || kneeWidth < hipWidth * 0.7) {
    return { isValgus: true, message: 'GÜán+Å Push your knees outward! Avoid caving in.' };
  }
...
export function validateCameraPositioning(keypoints, width = 640, height = 480) {
  if (!keypoints || keypoints.length === 0) {
    return { isPositioned: false, issue: '=ƒöì Searching for body in camera viewGÇª' };
  }
  ...
  if (missingJoints.length > 2) {
    return { isPositioned: false, issue: '=ƒô¦ Step back! Full body (head to feet) must be visible.' };
  }
  ...
  if (head && head.y < marginY) {
    return { isPositioned: false, issue: 'G¼ån+Å Adjust camera! Head is clipped at top.' };
  }
  if ((ankleL && ankleL.y > height - marginY) || (ankleR && ankleR.y > height - marginY)) {
    return { isPositioned: false, issue: 'G¼çn+Å Adjust camera! Feet are clipped at bottom.' };
  }

  return { isPositioned: true, issue: '=ƒƒó Perfect positioning! Ready to start.' };
}
