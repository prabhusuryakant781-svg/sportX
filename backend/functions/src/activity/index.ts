  ...
  try {
    ...
    if (logs.length > 0) {
      allItems = logs.map((l) => ({
        ...
        durationMinutes: Math.max(1, Math.round((l.durationSeconds || 60) / 60)),
        formScore: l.formScore,
        detectedErrors: l.detectedErrors || [],
        calories: l.calories,
        xpAwarded: (l as any).xpAwarded ?? (l as any).xpEarned ?? 0,
        xpEarned: (l as any).xpEarned ?? (l as any).xpAwarded ?? 0,
      ...
      allItems = filtered.map((s) => ({
        ...
        durationMinutes: s.durationMinutes || Math.max(1, Math.round((s.durationSeconds || 60) / 60)),
        formScore: s.formAccuracyAverage,
        detectedErrors: s.formErrors || [],
        calories: s.caloriesBurned,
        xpAwarded: s.xpEarned,
        xpEarned: s.xpEarned,
        status: s.status,
